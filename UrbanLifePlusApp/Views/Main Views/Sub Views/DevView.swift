            ...
            Section(header: Text("Sign In With Apple Details")) {
                VStack(alignment: .leading) {
                    ...
                    TextField("User ID", text: $userId, onEditingChanged: { _ in
                        self.userId = self.storageLocal.userId
                    })
                        .onAppear {
                            self.userId = self.storageLocal.userId
                    }
                ...
                VStack(alignment: .leading) {
                    ...
                    TextField("Identity Token", text: $identityToken, onEditingChanged: { _ in
                        self.identityToken = String(data: self.storageLocal.identityToken, encoding: String.Encoding.utf8) ?? ""
                    })
                        .onAppear {
                            self.identityToken = String(data: self.storageLocal.identityToken, encoding: String.Encoding.utf8) ?? ""
                    }
                ...
                VStack(alignment: .leading) {
                    ...
                    TextField("Authorization Code", text: $authorizationCode, onEditingChanged: { _ in
                        self.authorizationCode = String(data: self.storageLocal.authorizationCode, encoding: String.Encoding.utf8) ?? ""
                    })
                        .onAppear {
                            self.authorizationCode = String(data: self.storageLocal.authorizationCode, encoding: String.Encoding.utf8) ?? ""
                    }
